...
// Ancestors worth rerouting a descendant click to. Deliberately a SUBSET of
// INTERACTIVE_SELECTOR: a click on a descendant of a native control (button, a,
// input, summary, [onclick]) bubbles up and activates it, so rerouting that
// click to the ancestor center is a pure false positive — it would have worked.
// Those controls are excluded here. Only list-widget types — where the
// clickable part is often a sibling or a specific sub-region (a combobox's
// chevron, a select, a menu item) rather than the whole box — benefit from
// nudging the click to the element's center.
const BLOCK_ANCESTOR_SELECTOR = [
  "[role=combobox]",
  "[role=menuitem]",
  "[role=menuitemradio]",
  "[role=menuitemcheckbox]",
...
async function probeClickTarget(tabId, cx, cy) {
  try {
    ...
    const expr = `(() => {
      ...
      // Only reroute to a list-widget ancestor (BLOCK_ANCESTOR_SELECTOR), not a
      // native control — a descendant click on a button/a/input already works
      // via bubbling, so blocking it would be a false positive.
      const ancSEL = ${JSON.stringify(BLOCK_ANCESTOR_SELECTOR)};
      const anc = el.closest(ancSEL) || null;
